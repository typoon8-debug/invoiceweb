...
- 수락 기준과 완료 조건 정의
- **구현 완료 후 반드시 테스트를 수행해야 하며, 테스트 통과 전에는 다음 단계로 진행 불가**
- **API 연동 및 비즈니스 로직 구현 시 Playwright MCP를 활용한 E2E 테스트 필수**
- 테스트 항목:
  - API 호출 성공/실패 시나리오 검증
  - 비즈니스 로직 정상 동작 확인
  - 에러 핸들링 및 사용자 피드백 검증
  - 엣지 케이스 (빈 값, 잘못된 입력, 네트워크 오류 등) 테스트
  - 화면 렌더링 및 상태 변화 확인 (Playwright MCP 스크린샷/스냅샷 활용)
...
- 작업 파일의 명세서를 따름
- 기능과 기능성 구현
- **[필수] API 연동 및 비즈니스 로직 구현 시 Playwright MCP로 테스트 수행 필수**
- **[필수] 각 기능 구현 직후 반드시 테스트를 수행하고, 테스트 결과를 확인한 후 다음 구현으로 진행**
- Playwright MCP 테스트 절차:
  1. `mcp__playwright__browser_navigate`로 해당 페이지/엔드포인트 접근
  2. `mcp__playwright__browser_snapshot` 또는 `mcp__playwright__browser_take_screenshot`으로 초기 상태 확인
  3. 기능 동작 수행 (폼 입력, 버튼 클릭, API 호출 등)
  4. 결과 상태 검증 (성공 메시지, 데이터 반영, 에러 처리 등)
  5. `mcp__playwright__browser_network_requests`로 API 요청/응답 검증
  6. 에러 시나리오 및 엣지 케이스 테스트
- 각 단계 후 작업 파일 내 단계 진행 상황 업데이트
- 테스트 실패 시 원인을 분석하고 수정한 후 재테스트 수행
...
- **Task 005: 데이터베이스 및 API 개발** - 우선순위
  ...
  - **[구현 후 테스트 필수]** Playwright MCP로 각 API 엔드포인트 동작 검증
  - 테스트 항목: 정상 응답, 에러 응답, 데이터 정합성, 네트워크 요청/응답 확인

- **Task 006: 인증 및 권한 시스템 구현**
  - 사용자 인증 시스템 구축
  - 권한 기반 접근 제어 구현
  - 보안 미들웨어 및 세션 관리
  - **[구현 후 테스트 필수]** Playwright MCP로 인증 플로우 E2E 테스트 수행
  - 테스트 항목: 로그인/로그아웃, 권한 없는 접근 차단, 토큰 만료 처리

- **Task 006-1: 핵심 기능 통합 테스트 (Playwright MCP)**
  - Playwright MCP를 사용한 전체 사용자 플로우 E2E 테스트
  - 시나리오별 테스트: 정상 플로우, 에러 플로우, 엣지 케이스
  - API 연동 및 비즈니스 로직 최종 검증
  - `mcp__playwright__browser_network_requests`로 API 요청/응답 전수 검사
  - 테스트 실패 항목 수정 및 재검증 후 완료 처리
...
5. **테스트 필수**: API 연동 또는 비즈니스 로직이 포함된 Task는 반드시 다음을 포함
   - `[구현 후 테스트 필수]` 레이블 명시
   - Playwright MCP를 활용한 테스트 시나리오 (정상/에러/엣지 케이스)
   - 작업 파일 내 `## 테스트 체크리스트` 섹션 (각 시나리오를 체크리스트로 작성)
...
- [ ] API 연동 및 비즈니스 로직 구현 Task에 `[구현 후 테스트 필수]` 레이블이 표시되었는가?
- [ ] 각 작업 파일에 "## 테스트 체크리스트" 섹션이 명시되었는가?
- [ ] 모든 API 엔드포인트에 대한 정상/에러/엣지 케이스 시나리오가 정의되었는가?
- [ ] Playwright MCP 도구(`browser_navigate`, `browser_snapshot`, `browser_network_requests` 등) 사용이 명시되었는가?
- [ ] 구현 완료 후 테스트 수행 → 실패 시 수정 → 재테스트 흐름이 명확한가?
- [ ] Phase 3에 전체 통합 테스트 Task(Playwright MCP 기반 E2E)가 독립적으로 포함되었는가?
- [ ] 비즈니스 로직 테스트 시나리오에 에러 핸들링 및 사용자 피드백 검증이 포함되었는가?
- [ ] 네트워크 요청/응답 검증(`mcp__playwright__browser_network_requests`)이 API Task에 포함되었는가?
